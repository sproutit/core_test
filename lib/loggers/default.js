...
Ct.DefaultLogger = utils.extend(/** @scope Ct.DefaultLogger.prototype */{
  ...
  */
    ...
    if (hasConsole && console.groupEnd) {
      ...
      if (state.moduleName) console.groupEnd(state.moduleName);
  ...
  */
  add: function(status, testInfo, message) {
    ...
    if (state.moduleName !== moduleName) {
      ...
      // exit current module if there is one
      if (state.moduleName && console.groupEnd) {
        console.groupEnd(state.testName);
        console.groupEnd(status.moduleName);
      }
