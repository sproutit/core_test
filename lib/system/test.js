...
Ct.Test = utils.extend(Assertable, {
  ...
  */
  _phase: function(mode, handler, ctx) {
    var test = this;
    return function(done) {
      ...
      // initialize test state during setup mode only
      if (mode === Ct.SETUP_MODE) {
        test._actualAsserts = 0 ;
        test._expectedAsserts = -1;
      }

      // invoked when test phase completes
      var cleanup = function() {
        test._cancelTimeout();

        if ((mode===Ct.TEARDOWN_MODE) && !test.didFail) test._verifyAsserts();
