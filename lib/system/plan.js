...
Ct.Plan = utils.extend({
  ...
  */
  ...
  /**
    ...
    @returns {Function} new schedule promise to replace the one passed in
  */
  schedule: function(filter) {
    ...
    for(idx=0;idx<len;idx++) {
      task = modules[idx].schedule(filter);
    ...
    return function(done) {
      plan._begin();
      var allPassed = true;
      utils.each(queue, function(task, done) {
        task(function(err, passed) {
        ...
        else return done(null, allPassed);
      });
    };
  },
