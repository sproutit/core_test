...
CoreTest = {
  ...
  VERSION: (module.ownerPackage ? module.ownerPackage.get('version'):null),
  ...
  */
  run: function(plan, filter) {
    ...
    this.then(this, plan.schedule(filter));
  ...
  /**
    Adds a handler to the schedule and starts running the schedule if needed.
    
    @param {Object} context 
      (Optional) context object
    
    @param {Function} handler
      Handler to invoke next on the schedule
  */
  then: function(context, handler) {
    var self = this, goNext;

    if (!this._schedule) this._schedule = [];
    this._schedule.push({ context: context, method: handler });
    
    if (!this._running) {
      this._running = true;
      goNext = function() {
        var next = self._schedule.shift();
        if (next) next.method.call(next.context, goNext);
...
// if in browser...    
if ((T_UNDEFINED !== typeof window) && (T_UNDEFINED !== typeof document)) {
   DefaultLogger = require('loggers/browser');
} else {
  DefaultLogger = require('loggers/node');
}
